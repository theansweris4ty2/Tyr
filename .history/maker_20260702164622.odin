...
for i in 0..<8 {
 ...
 defer delete(terrain_tiles)
    ...
    append(&terrain_tiles, Tile{{x, y, TILE_WIDTH, TILE_HEIGHT}, "", {}, 5 , 5})
}
for i in 0..<500 {   
    for &terrain_tile in terrain_tiles{
        terrain_tile.rect.x += terrain_tile.x_velocity
         terrain_tile.rect.y += terrain_tile.y_velocity
         fmt.println(terrain_tiles[10].rect.x, terrain_tiles[10].x_velocity, terrain_tiles[10].y
        )
         ...
         for tile in water {
            if check_collisions(tile.rect, terrain_tile.rect){
                append(&water, Tile{terrain_tile.rect,"water", texture, 0, 0 })
