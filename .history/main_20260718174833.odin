...
if action == "sell"{
        ...
        if rl.CheckCollisionPointRec(point, button.rect) && rl.IsMouseButtonPressed(.LEFT){
            ...
            case "Grain":
                if p_ptr.grain > 0 {
                    goods = "Grain"
                    sell_goods(p_ptr, goods, 1, market["Grain"], 1)
                    p_ptr.grain -= 1
                }
                
            case "Lumber":
                if p_ptr.lumber > 0 {
                    sell_goods(p_ptr, market["Lumber"], 1)
                }
            case "Ore":
                if p_ptr.ore > 0 {
                    sell_goods(p_ptr, market["Ore"], 1)
                    p_ptr.ore -= 1
    ...
    if map_screen {
        ...
        // TODO: Write logic for battles including end conditions or else logic below will only work the first time
        if tile.invaded && tile.occupied && !battle_ended{
        battle_screen = true
    }
}
