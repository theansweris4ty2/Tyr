...
for i in 0..<8 {
 ...
 defer delete(terrain_tiles)
    for i in 0..<500 {
    x:= rand.float32_range(0, 500)
    y:= rand.float32_range(0,500)
    append(&terrain_tiles, Tile{{x, y, 300, 300}, "", {}, x_velocity , y_velocity})
    
}
fmt.println(len(terrain_tiles))
for i in 0..<500 {   
        ...
        //  }
         for tile in water {
            ...
            if check_collisions(tile.rect, terrain_tile.rect){
                append(&water, Tile{{terrain_tile.rect.x, terrain_tile.rect.y, terrain_tile.rect.w, terrain_tile.rect.h},"water", texture, 0, 0 })
