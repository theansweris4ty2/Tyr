...
generate_map::proc(texture: rl.Texture) -> [dynamic]Tile
{
    game_board: [dynamic]Tile
        ...
        for j in 0..<432 {
            ...
            append(&game_board, Tile{{x, y, TILE_WIDTH, TILE_HEIGHT}, "farm", texture, production_value, false, false, false, {rl.BLACK, 0}})
        }
    return game_board
}
...
seed_tiles::proc(texture1: rl.Texture, texture2: rl.Texture, texture3: rl.Texture) -> ([dynamic; 50]Tile, [dynamic; 30]Tile, [dynamic; 30]Tile){
    ...
    for i in 0..<50{
        ...
        append(&water_tiles, Tile{{x, y, TILE_WIDTH, TILE_HEIGHT}, "water", texture1, 0, false, false, false, {rl.BLACK, 0}})
    }
    for i in 0..<30 {
        x:= rand.float32_range(0, 24) * 50
        y:= rand.float32_range(0, 18) * 50
         append(&forest_tiles, Tile{{x, y, TILE_WIDTH, TILE_HEIGHT}, "forest", texture3, 0, false, false, false, {rl.BLACK, 1}})
        
    }
    for i in 0..<30 {
        x:= rand.float32_range(0, 24) * 50
        y:= rand.float32_range(0, 18) * 50
        append(&ore_tiles, Tile{{x, y, TILE_WIDTH, TILE_HEIGHT}, "ore", texture3, 0, false, false, false, {rl.BLACK, 1}})
