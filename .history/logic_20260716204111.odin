...
// TODO: Try to change main proc so that player action is only triggered when an action is taken - could add if statement - this might reduce the overhead
...
draw_ui :: proc(menu: bool, buttons_array:.. [dynamic; 20]Button){
    if menu {
        rl.DrawRectangle(i32(menu1.rect.x), i32(menu1.rect.y), i32(menu1.rect.width), i32(menu1.rect.height), KHAKI)
        rl.DrawRectangleLinesEx(menu1.rect, 5, rl.BLACK)
        rl.DrawText("PLAYER ACTIONS",i32(menu1.rect.x + (menu1.rect.width/5)), i32(menu1.rect.y + 10), 50, rl.BLACK)
    }
...
rl.DrawRectangle(0, 0, 1500, 60, KHAKI)
rl.DrawText(strings.to_cstring(&b), 10, 20, 40, rl.BLACK)
}

// Add print_battle_stats proc that will print how many of each troop type you have to place in battle and battle stats. 
