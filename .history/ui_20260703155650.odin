...
draw_ui ::proc(menu: Menu, buttons: [dynamic; 6]Button, index1: i32, index2: i32, index3: i32, action1: i32, action2: i32, action3: i32, stats: Menu, player: Player){
    ...
    k2.DrawRectangle({button.rect.x, button.rect.y, button.rect.w, button.rect.h}, button.color)
    k2.DrawText(button.label, {button.rect.x + 1, button.rect.y + 10}, 30, k2.BLACK)
    k2.DrawRectangle_outline(button.rect, 1, k2.BLACK)
    } 
    for button in buttons {
    k2.DrawRectangle({button.rect.x, button.rect.y, button.rect.w, button.rect.h}, button.color)
    k2.DrawText(button.label, {button.rect.x + 10, button.rect.y + 10}, 40, k2.BLACK)
    }
    for button in stats.buttons {
        
    k2.DrawRectangle({button.rect.x, button.rect.y, button.rect.w, button.rect.h}, button.color)
    k2.DrawText(button.label, {button.rect.x + 10, button.rect.y + 10}, 30, k2.BLACK)
    k2.DrawRectangle_outline(button.rect, 1, k2.BLACK)
    }
    dice_images := load_game_textures(dice_files)
    k2.DrawTexture(dice_images[index1], {0, 700})
    k2.DrawTexture(dice_images[index2], {100, 700})
    k2.DrawTexture(dice_images[index3], {200, 700})
    k2.DrawRectangle_outline(menu.rect, menu.border, k2.BLACK)
    k2.DrawText(menu.label, {menu.rect.x + 50, menu.rect.y + 10}, 70, k2.BLACK)
    k2.DrawRectangle_outline(stats.rect, stats.border, k2.BLACK)
    k2.DrawText(stats.label, {stats.rect.x + 50, stats.rect.y + 10}, 70, k2.BLACK)
    k2.DrawText(fmt.tprintf("Action 1: {}", action1), {menu.rect.x + 50, menu.rect.y + 100}, 50, k2.BLACK)
    k2.DrawText(fmt.tprintf("Action 2: {}", action2), {menu.rect.x + 50, menu.rect.y + 150}, 50, k2.BLACK)
    k2.DrawText(fmt.tprintf("Action 3: {}", action3), {menu.rect.x + 50, menu.rect.y + 200}, 50, k2.BLACK)
    k2.DrawText(fmt.tprintf("%d", player.crops), {stats.rect.x + 200, stats.rect.y + 100}, 50, k2.BLACK)
    k2.DrawText(fmt.tprintf("%d", player.lumber), {stats.rect.x + 200, stats.rect.y + 150}, 50, k2.BLACK)
    k2.DrawText(fmt.tprintf("%d", player.ore), {stats.rect.x + 200, stats.rect.y + 200}, 50, k2.BLACK)
    k2.DrawText(fmt.tprintf("%d", player.troops), {stats.rect.x + 200, stats.rect.y + 250}, 50, k2.BLACK)
