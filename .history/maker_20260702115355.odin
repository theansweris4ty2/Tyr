...
seed :: proc(texture: k2.Texture, texture2: k2.Texture, texture3: k2.Texture) -> ([dynamic; 15]Tile, [dynamic; 15]Tile, [dynamic; 15]Tile){

water : [dynamic; 15]Tile 
forests : [dynamic; 15]Tile
...
for i in 0..<15 {
 ...
 for i in 0..<15 {
    x = rand.float32_range(10, 1500)
    y = rand.float32_range(10, 800)
    append(&forests, Tile{{x, y, f32(50), f32(100)}, "forest", texture3, 5, 5})
 }
...
spread :: proc(farm_tiles: [dynamic; 15]Tile, forest_tiles:[dynamic; 15]Tile, water_tiles: [dynamic; 15]Tile) -> [dynamic]Tile{
...
for &tile, i in tile_map{
    for tile in tile_map{
        ...
        // if tile_map[i].rect.x + tile_map[i].rect.w >= tile_map[i+1].rect.x && tile_map[i].rect.y + tile_map[i].rect.h >= tile_map[i+1].rect.y && tile_map[i].rect.x <= tile_map[i+1].rect.x + tile_map[i+1].rect.w{
        //     fmt.println("hit")
        //     unordered_remove(&tile_map, i)
        // }
