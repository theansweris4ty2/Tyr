...
seed_tiles::proc(texture1: rl.Texture, texture2: rl.Texture, texture3: rl.Texture) -> ([dynamic; 200]Tile, [dynamic; 30]Tile, [dynamic; 30]Tile){
    x: f32
    y: f32
    water_tiles: [dynamic;200]Tile
    ...
    for i in 0..<30 {
        ...
        y:= rand.float32_range(0, 18) * 50
         append(&forest_tiles, Tile{{x, y, TILE_WIDTH, TILE_HEIGHT}, "forest", texture3, 0, 0})
        
    }
    for i in 0..<30 {
        x:= rand.float32_range(0, 24) * 50
        y:= rand.float32_range(0, 18) * 50
        append(&ore_tiles, Tile{{x, y, TILE_WIDTH, TILE_HEIGHT}, "ore", texture3, 0, 0})
