...
*/
player_action :: proc(texture: rl.Texture, texture2: rl.Texture, tile_map: [dynamic]Tile, player_ptr: ^Player, camera: rl.Camera2D){
    
    point := rl.GetWorldToScreen2D(rl.GetMousePosition(), camera)
...
camera_movement :: proc(camera: ^rl.Camera2D){
    if rl.IsKeyDown(.RIGHT){
        if camera.target.x < 700{
            camera.target.x += 1
        }
    } if rl.IsKeyDown(.LEFT){
        if camera.target.x > 100 {
            camera.target.x -= 1
        }
        fmt.println(camera.target.x)
    }
     if rl.IsKeyDown(.DOWN){
        if camera.target.y < 610 {
        camera.target.y += 1
        }
        fmt.println(camera.target.y)
    }
     if rl.IsKeyDown(.UP){
        if camera.target.y > 95 {
        camera.target.y -= 1
        }
