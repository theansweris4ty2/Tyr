...
player_action :: proc(tile_map: [dynamic]Tile, player_ptr: ^Player, point: rl.Vector2, action: string, town_texture: rl.Texture2D, menu_state: bool){
    ...
    if rl.IsMouseButtonPressed(.RIGHT) && rl.CheckCollisionPointRec(point, {tile.rect.x, tile.rect.y,tile.rect.width - 10, tile.rect.height - 10}){
        fmt.printf("%s: %d, tile number: %d \n occupied: %v \n", tile.kind, tile.production_value, i, tile.occupied)
...
generate_map::proc(texture: rl.Texture, water: rl.Texture, forest: rl.Texture, ore: rl.Texture) -> [dynamic]Tile
{
    ...
    invaded: bool
        for j in 0..<1512{
            ...
            invaded := rand.int32_range(1, 100)
            ...
            append(&game_board, Tile{{x, y, TILE_WIDTH, TILE_HEIGHT}, "farm", texture, production_value, false, occupied, false, {rl.BLACK, 1}})
        }
