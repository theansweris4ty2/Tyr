...
draw_ui ::proc(menu: Menu, buttons: [dynamic; 6]Button, index1: i32, index2: i32, index3: i32, action1: i32, action2: i32, action3: i32, stats: Menu, player: Player){
    ...
    rl.draw_rect({button.rect.x, button.rect.y, button.rect.w, button.rect.h}, button.color)
    rl.draw_text(button.label, {button.rect.x + 1, button.rect.y + 10}, 30, rl.BLACK)
    rl.draw_rect_outline(button.rect, 1, rl.BLACK)
    } 
    for button in buttons {
    rl.draw_rect({button.rect.x, button.rect.y, button.rect.w, button.rect.h}, button.color)
    rl.draw_text(button.label, {button.rect.x + 10, button.rect.y + 10}, 40, rl.BLACK)
    }
    for button in stats.buttons {
        
    rl.draw_rect({button.rect.x, button.rect.y, button.rect.w, button.rect.h}, button.color)
    rl.draw_text(button.label, {button.rect.x + 10, button.rect.y + 10}, 30, rl.BLACK)
    rl.draw_rect_outline(button.rect, 1, rl.BLACK)
    }
    dice_images := load_game_textures(dice_files)
    rl.draw_texture(dice_images[index1], {0, 700})
    rl.draw_texture(dice_images[index2], {100, 700})
    rl.draw_texture(dice_images[index3], {200, 700})
    rl.draw_rect_outline(menu.rect, menu.border, rl.BLACK)
    rl.draw_text(menu.label, {menu.rect.x + 50, menu.rect.y + 10}, 70, rl.BLACK)
    rl.draw_rect_outline(stats.rect, stats.border, rl.BLACK)
    rl.draw_text(stats.label, {stats.rect.x + 50, stats.rect.y + 10}, 70, rl.BLACK)
    rl.draw_text(fmt.tprintf("Action 1: {}", action1), {menu.rect.x + 50, menu.rect.y + 100}, 50, rl.BLACK)
    rl.draw_text(fmt.tprintf("Action 2: {}", action2), {menu.rect.x + 50, menu.rect.y + 150}, 50, rl.BLACK)
    rl.draw_text(fmt.tprintf("Action 3: {}", action3), {menu.rect.x + 50, menu.rect.y + 200}, 50, rl.BLACK)
    rl.draw_text(fmt.tprintf("%d", player.crops), {stats.rect.x + 200, stats.rect.y + 100}, 50, rl.BLACK)
    rl.draw_text(fmt.tprintf("%d", player.lumber), {stats.rect.x + 200, stats.rect.y + 150}, 50, rl.BLACK)
    rl.draw_text(fmt.tprintf("%d", player.ore), {stats.rect.x + 200, stats.rect.y + 200}, 50, rl.BLACK)
    rl.draw_text(fmt.tprintf("%d", player.troops), {stats.rect.x + 200, stats.rect.y + 250}, 50, rl.BLACK)
