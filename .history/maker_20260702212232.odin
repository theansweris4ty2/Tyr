...
terrain_tiles :[dynamic; 300]k2.Rect

for i in 0..<300 {
    x := rand.float32_range(0, 1600) 
    y := rand.float32_range(0, 1000) 
    append(&terrain_tiles, k2.Rect{x, y, TILE_WIDTH, TILE_HEIGHT})
...
for tile in tile_map{
    ...
    for rect in terrain_tiles{
        if check_collisions(tile.rect, rect){
            append(&tile_map, Tile{{rect.x, rect.y, rect.w, rect.h}, tile.kind, tile.texture, 0, 0}  )
        }      
    }
}
