...
for &tile in tile_map{
    for i in 0..=len(tile_map) -2 {
        if tile_map[i].rect.x + tile_map[i].rect.w >= tile_map[i+1].rect.x && tile_map[i].rect.y + tile_map[i].rect.h >= tile_map[i+1].rect.y && tile_map[i].rect.x <= tile_map[i+1].rect.x +{
            fmt.println("hit")
